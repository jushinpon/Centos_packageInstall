...
for my $GV (@GCC_Version){
	...
	$mattached_path = "$mpichPath$GV/bin";#attached path in main script
	path_setting($mattached_path);
	$mattached_ld = "$mpichPath$GV/lib";#attached ld path in main script
...
#sleep(100);
	...
	@lmp_package= ("kspace","manybody","molecule","user-meamc","user-misc","user-omp","rigid","misc","dipole","replica");
