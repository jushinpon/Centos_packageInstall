...
if ($? != 0){die "wget -O $currentVer $URL failed\n";} 
print $Check "$?:wget -O $currentVer $URL\n";

if(! (-e "$current_path/$Dir4download/$currentVer")){die "No $currentVer downloaded";}# if no mpich file
